...
function renderPowerlineStatusLine(
    ...
    const powerlineConfig = settings.powerline as
    ...
    const invertBgs
    = (config.separatorInvertBackground as boolean[] | undefined)
        ?? separators.map(() => false);
    ...
    const startCap
    = startCaps.length > 0 ? startCaps[capLineIndex % startCaps.length] : '';
    const endCap
    = endCaps.length > 0 ? endCaps[capLineIndex % endCaps.length] : '';
    ...
    if (themeName && themeName !== 'custom') {
        ...
        if (theme) {
            const colorLevel = getColorLevelString(
                settings.colorLevel as number as 0 | 1 | 2 | 3
            );
            const colorLevelKey
        = colorLevel === 'ansi16' ? '1' : colorLevel === 'ansi256' ? '2' : '3';
    ...
    const filteredWidgets = widgets.filter(
        widget => widget.type !== 'separator' && widget.type !== 'flex-separator'
    );
    ...
    for (let i = 0; i < filteredWidgets.length; i++) {
        ...
        const preRendered
      = actualPreRenderedIndex !== undefined
          ? preRenderedWidgets[actualPreRenderedIndex]
          : undefined;
        ...
        if (widgetText) {
            ...
            const nextItem
        = i < filteredWidgets.length - 1 ? filteredWidgets[i + 1] : null;
        ...
        = widget.type === 'custom-command' && widget.preserveColors;
            ...
            if (themeColors) {
                if (!skipFgTheme) {
                    fgColor
            = themeColors.fg[widgetColorIndex % themeColors.fg.length] ?? fgColor;
                }
                bgColor
          = themeColors.bg[widgetColorIndex % themeColors.bg.length] ?? bgColor;
        ...
        for (let i = 0; i < widgetElements.length; i++) {
            ...
            if (!isPreviousMerged) {
                ...
                if (maxWidth !== undefined) {
                    ...
                    let combinedLength = stringWidth(
                        element.content.replace(ANSI_REGEX, '')
                    );
                    let j = i;
                    while (
                        j < widgetElements.length - 1
                        && widgetElements[j]?.widget.merge
                    ) {
                        j++;
                        const nextElement = widgetElements[j];
                        if (nextElement) {
                            combinedLength += stringWidth(
                                nextElement.content.replace(ANSI_REGEX, '')
                            );
                        }
                    }
    ...
    for (let i = 0; i < widgetElements.length; i++) {
      ...
      = i < widgetElements.length - 1
        ...
        const isPreserveColors
      = widget.widget.type === 'custom-command' && widget.widget.preserveColors;
        ...
        = widget.bgColor
            && nextWidget.bgColor
            && widget.bgColor === nextWidget.bgColor;
    ...
    for (const lineWidgets of allLinesWidgets) {
        ...
        for (const widget of lineWidgets) {
            ...
            try {
                ...
                // Widget render failed - use empty string to skip in merge groups
                widgetText = '';
            }
...
export function calculateMaxWidthsFromPreRendered(
    ...
    for (const preRenderedLine of preRenderedLines) {
        ...
        for (let i = 0; i < filteredWidgets.length; i++) {
            ...
            let j = i;
            while (
                j < filteredWidgets.length - 1
                && filteredWidgets[j]?.widget.merge
            ) {
                j++;
...
export function renderStatusLine(
    ...
    const powerlineSettings = settings.powerline as
        | Record<string, unknown>
        | undefined;
    ...
    // Override foreground color takes precedence over EVERYTHING, including passed foreground color
        ...
        if (
            ...
            && settings.overrideBackgroundColor !== 'none'
        ) {
    ...
    for (let i = 0; i < widgets.length; i++) {
        ...
        if (widget.type === 'separator') {
            ...
            for (let j = i - 1; j >= 0; j--) {
                ...
                if (
                    prevWidget
                    && prevWidget.type !== 'separator'
                    && prevWidget.type !== 'flex-separator'
                ) {
            ...
            if (
                settings.inheritSeparatorColors
                && i > 0
                && !widget.color
    ...
    const defaultSep = settings.defaultSeparator
        ? formatSeparator(settings.defaultSeparator)
        : '';
    ...
    // Add default separator between any two items (but not before first item, and not around flex separators)
      ...
      = defaultSep
        ...
        if (elem.type === 'separator' || elem.type === 'flex-separator') {
            ...
            const omitTrailingPadding
        = elem.widget?.merge === 'no-padding' && nextElem;
        ...
        || Boolean(
            ...
            if (padding && (elem.widget?.backgroundColor || hasColorOverride)) {
                ...
                const leadingPadding = omitLeadingPadding
                    ? ''
                    : applyColorsWithOverride(
                        padding,
                        undefined,
                        elem.widget?.backgroundColor
                    );
    ...
    // No flex separator OR no width detected
        if (hasFlexSeparator && !terminalWidth) {
            ...
            statusLine = finalElements
                .map(e => (e === 'FLEX' ? chalk.gray(' | ') : e))
                .join('');
        } else {
