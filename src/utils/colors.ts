...
export function getAvailableColorsForUI(): { name: string; value: string }[] {
    ...
    return [
        { name: 'Default', value: '' },
        ...COLOR_MAP.filter(c => !c.isBackground).map(c => ({
            name: c.displayName,
            value: c.name
        }))
    ];
...
export function getAvailableBackgroundColorsForUI(): {
    name: string;
    value: string;
}[] {
...
export function getBackgroundColorsForPowerline(): string[] {
    ...
    return COLOR_MAP.filter(c => c.isBackground && c.name !== 'bgBlack').map(
        c => c.name
    );
}
