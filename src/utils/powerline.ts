    ...
    if (platform === "linux" || platform === "darwin") {
      try {
        ...
        const { stdout } = await execAsync(
          "fc-list 2>/dev/null | grep -i powerline",
          { encoding: "utf8" },
        );
 ...
 */
export async function installPowerlineFonts(): Promise<{
  success: boolean;
  message: string;
}> {
  // Ensure this is always async
  ...
  try {
    ...
    const tempDir = path.join(
      os.tmpdir(),
      `ccstatusline-powerline-fonts-${Date.now()}`,
    );
    ...
    if (errorMessage.includes("git")) {
      return {
        success: false,
        message:
          "Git is required to install Powerline fonts. Please install Git and try again.",
      };
    }
