...
export const PowerlineThemeSelector: React.FC<PowerlineThemeSelectorProps> = ({
    ...
    if (themeColors) {
      ...
      const newLines = settings.lines.map((line) => {
        ...
        return line.map((widget) => {
          ...
          const fgColor =
            themeColors.fg[widgetColorIndex % themeColors.fg.length];
          const bgColor =
            themeColors.bg[widgetColorIndex % themeColors.bg.length];
          widgetColorIndex++;
  ...
  const selectedTheme = selectedThemeName
    ? getPowerlineTheme(selectedThemeName)
    : undefined;
    ...
    return (
      <Box flexDirection="column">
        ...
        <Box marginTop={1} flexDirection="column">
          ...
          <Text color="red">
            This will overwrite any existing custom colors!
          </Text>
        </Box>
        <Box marginTop={2}>
          <Text>Continue?</Text>
      ...
      {selectedTheme && (
        <Box marginTop={2} flexDirection="column">
          ...
          {settings.colorLevel === 1 && (
            <Box>
              <Text color="yellow">
                ⚠ 16 color mode themes have a very limited palette, we recommend
                switching color level in Terminal Options
              </Text>
            </Box>
          )}
        </Box>
