...
export const ItemsEditor: React.FC<ItemsEditorProps> = ({
  ...
  const getAllowedTypes = (): WidgetItemType[] => {
    ...
    if (settings.powerline.enabled) {
      allowedTypes = allowedTypes.filter(
        (t) => t !== "separator" && t !== "flex-separator",
      );
    }
  ...
  const getDefaultItemType = (): WidgetItemType => {
    ...
    return allowedTypes.includes("model")
      ? "model"
      : (allowedTypes[0] ?? "model");
  };

  // Get a unique background color for powerline mode
  const getUniqueBackgroundColor = (
    insertIndex: number,
  ): string | undefined => {
    ...
    const nextWidget =
      insertIndex < widgets.length ? widgets[insertIndex] : null;
    ...
    const availableColors = bgColors.filter(
      (color) => color !== prevBg && color !== nextBg,
    );
    ...
    if (moveMode) {
      ...
      if (key.upArrow) {
        ...
        if (currentWidget && currentWidget.type === "separator") {
          ...
          const nextChar =
            separatorChars[(currentCharIndex + 1) % separatorChars.length];
          const newWidgets = [...widgets];
        ...
        if (
          ...
          newWidgets[selectedIndex] = {
            ...currentWidget,
            rawValue: !currentWidget.rawValue,
          };
          onUpdate(newWidgets);
        }
        ...
        if (
          ...
          if (widgetImpl) {
            if (widgetImpl.getCustomKeybinds) {
              ...
              const matchedKeybind = customKeybinds.find(
                (kb) => kb.key === input,
              );
  ...
  const hasFlexSeparator = widgets.some(
    (widget) => widget.type === "flex-separator",
  );
    ...
    <Box flexDirection="column">
      ...
      {moveMode ? (
        <Box flexDirection="column" marginBottom={1}>
          <Text dimColor>
            ↑↓ to move widget, ESC or Enter to exit move mode
          </Text>
        </Box>
      ) : (
        <Box flexDirection="column">
      ...
      {hasFlexSeparator && !widthDetectionAvailable && (
        <Box marginTop={1}>
          <Text color="yellow">
            ⚠ Note: Terminal width detection is currently unavailable in your
            environment.
          </Text>
          <Text dimColor>
            {" "}
            Flex separators will act as normal separators until width detection
            is available.
          </Text>
        </Box>
      )}
            ...
            {widgets.map((widget, index) => {
              ...
              const widgetImpl =
                widget.type !== "separator" && widget.type !== "flex-separator"
                  ...
                  : null;
              const { displayText, modifierText } =
                widgetImpl?.getEditorDisplay(widget) ?? {
              ...
              return (
                <Box key={widget.id} flexDirection="row" flexWrap="nowrap">
                  <Box width={3}>
                    <Text
                      color={
                        isSelected ? (moveMode ? "blue" : "green") : undefined
                      }
                    >
                      {isSelected ? (moveMode ? "◆ " : "▶ ") : "  "}
                    </Text>
                  ...
                  {widget.merge === "no-padding" && (
                    <Text dimColor> (merged-no-pad→)</Text>
                  )}
                </Box>
              );
            })}
