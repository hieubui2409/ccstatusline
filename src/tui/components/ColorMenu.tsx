...
export const ColorMenu: React.FC<ColorMenuProps> = ({
    ...
    if (hexInputMode) {
      ...
      if (key.escape) {
        ...
        if (hexInput.length === 6) {
          ...
          const selectedWidget = colorableWidgets.find(
            (widget) => widget.id === highlightedItemId,
          );
          if (selectedWidget) {
          ...
          const newItems = widgets.map((widget) => {
            if (widget.id === selectedWidget.id) {
              if (editingBackground) {
                ...
                if (key.rightArrow) {
                  nextBgColorIndex =
                    (currentBgColorIndex + 1) % bgColors.length;
                } else {
                  nextBgColorIndex =
  ...
  const currentColor = editingBackground
    ...
    : selectedWidget
      ? (selectedWidget.color ??
        (() => {
          if (
            selectedWidget.type !== "separator" &&
            selectedWidget.type !== "flex-separator"
          ) {
  ...
  if (editingBackground) {
    if (!currentColor || currentColor === "") {
      ...
      if (currentColor.startsWith("ansi256:")) {
        ...
        const colorOption = bgColorOptions.find(
    ...
    return (
      <Box flexDirection="column">
        ...
        <Box marginTop={1} flexDirection="column">
          <Text>
            This will reset all colors for all widgets to their defaults.
          </Text>
  ...
  const hasGlobalBgOverride =
    !!settings.overrideBackgroundColor && !powerlineEnabled;
    ...
    <Box flexDirection="column">
      ...
      {hexInputMode ? (
        <Box flexDirection="column">
          ...
          <Text>
            #{hexInput}
            <Text dimColor>
              {hexInput.length < 6 ? "_".repeat(6 - hexInput.length) : ""}
            </Text>
          </Text>
          <Text> </Text>
        ...
        <>
          <Text dimColor>
            ↑↓ to select, ←→ to cycle{" "}
            {editingBackground ? "background" : "foreground"}, (f) to toggle
            bg/fg, (b)old,
            {settings.colorLevel === 3
              ? " (h)ex,"
              : settings.colorLevel === 2
      ...
      <Box marginTop={1} flexDirection="column">
        ...
        <Text dimColor wrap="wrap">
          If colors appear incorrect in the VSCode integrated terminal, the
          "Terminal › Integrated: Minimum Contrast Ratio"
          (`terminal.integrated.minimumContrastRatio`) setting is forcing a
          minimum contrast between foreground and background colors. You can
          adjust this setting to 1 to disable the contrast enforcement, or use a
          standalone terminal for accurate colors.
        </Text>
      </Box>
    </Box>
  );
