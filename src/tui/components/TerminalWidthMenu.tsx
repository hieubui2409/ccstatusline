...
export const TerminalWidthMenu: React.FC<TerminalWidthMenuProps> = ({
  ...
  const [selectedOption, setSelectedOption] = useState<FlexMode>(
    settings.flexMode,
  );
    ...
    if (editingThreshold) {
      if (key.return) {
        ...
        if (isNaN(value)) {
          ...
          setValidationError(
            `Value must be between 1 and 99 (you entered ${value})`,
          );
        } else {
  ...
  return (
    <Box flexDirection="column">
      ...
      <Text color="white">
        These settings affect where long lines are truncated, and where
        right-alignment occurs when using flex separators
      </Text>
      <Text dimColor wrap="wrap">
        Claude code does not currently provide an available width variable for
        the statusline and features like IDE integration, auto-compaction
        notices, etc all cause the statusline to wrap if we do not truncate it
      </Text>
