...
export const TerminalOptionsMenu: React.FC<TerminalOptionsMenuProps> = ({
    ...
    const [pendingColorLevel, setPendingColorLevel] = useState<
    0 | 1 | 2 | 3 | null
    >(null);
    ...
    const handleSelect = () => {
        if (selectedIndex === 2) {
            ...
            const hasCustomColors = settings.lines.some((line: WidgetItem[]) => line.some(
                (widget: WidgetItem) => Boolean(
                    widget.color
                    && (widget.color.startsWith('ansi256:')
                        || widget.color.startsWith('hex:'))
                )
                || Boolean(
        ...
        <Box flexDirection='column'>
            ...
            {showColorWarning ? (
                <Box flexDirection='column' marginTop={1}>
                    ...
                    <Text>
                        Switching color modes will reset custom ansi256 or hex colors to
                        defaults.
                    </Text>
                    <Box marginTop={1}>
                        <ConfirmDialog
