...
export const GlobalOverridesMenu: React.FC<GlobalOverridesMenuProps> = ({
  settings,
  onUpdate,
  onBack,
}) => {
  ...
  const [inheritColors, setInheritColors] = useState(
  ...
  const hasManualSeparators = settings.lines.some((line) =>
    line.some((item) => item.type === "separator"),
  );

  // Get colors from COLOR_MAP
  const bgColors = [
    "none",
    ...COLOR_MAP.filter((c) => c.isBackground).map((c) => c.name),
  ];
  const fgColors = [
    "none",
    ...COLOR_MAP.filter((c) => !c.isBackground).map((c) => c.name),
  ...
  const currentBgIndex = bgColors.indexOf(
    settings.overrideBackgroundColor ?? "none",
  );
  const currentFgIndex = fgColors.indexOf(
      ...
      if (key.escape) {
        ...
        const updatedSettings = {
          ...
          overrideBackgroundColor:
            nextBgColor === "none" ? undefined : nextBgColor,
        };
        onUpdate(updatedSettings);
        ...
        const updatedSettings = {
          ...
          overrideForegroundColor:
            nextFgColor === "none" ? undefined : nextFgColor,
        };
        onUpdate(updatedSettings);
  ...
  return (
    <Box flexDirection="column">
      ...
      <Text dimColor>
        Configure automatic padding and separators between widgets
      </Text>
      {isPowerlineEnabled && (
        <Box marginTop={1}>
          <Text color="yellow">
            ⚠ Some options are disabled while Powerline mode is active
          </Text>
        </Box>
      )}
      <Box marginTop={1} />
      ...
      {editingPadding ? (
        <Box flexDirection="column">
          <Box>
            <Text>
              Enter default padding (applied to left and right of each
              widget):{" "}
            </Text>
            <Text color="cyan">
              {paddingInput ? `"${paddingInput}"` : "(empty)"}
            </Text>
          </Box>
        ...
        <Box flexDirection="column">
          <Box>
            ...
            <Text color="cyan">
              {separatorInput
                ? `"${separatorInput}"`
                : "(empty - no separator will be added)"}
            </Text>
          </Box>
          <Text dimColor>Press Enter to save, ESC to cancel</Text>
        </Box>
        ...
        <Box flexDirection="column">
          <Box marginBottom={1}>
            <Text color="yellow">
              ⚠ Warning: Setting a default separator will remove all existing
              manual separators from your status lines.
            </Text>
          </Box>
          <Box>
            <Text>New default separator: </Text>
            <Text color="cyan">
              {separatorInput ? `"${separatorInput}"` : "(empty)"}
            </Text>
          </Box>
          <Box marginTop={1}>
        ...
        <>
          <Box>
            ...
            <Text color={globalBold ? "green" : "red"}>
              {globalBold ? "✓ Enabled" : "✗ Disabled"}
            </Text>
          ...
          <Box marginTop={1} flexDirection="column">
            <Text dimColor wrap="wrap">
              ...
              widgets to your widget list.
            </Text>
            <Text dimColor wrap="wrap">
              • Inherit colors: Separators will use colors from the preceding
              widget
            </Text>
            <Text dimColor wrap="wrap">
              • Global Bold: Makes all text bold regardless of individual
              settings
            </Text>
            <Text dimColor wrap="wrap">
              • Override colors: All widgets will use these colors instead of
