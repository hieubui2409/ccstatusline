...
export const PowerlineSeparatorEditor: React.FC<
    PowerlineSeparatorEditorProps
> = ({ settings, mode, onUpdate, onBack }) => {
    ...
    const invertBgs
    = mode === 'separator' ? powerlineConfig.separatorInvertBackground : [];
    ...
    const getSeparatorDisplay = (char: string, index: number): string => {
        ...
        if (preset) {
            ...
            const inversionText
        = mode === 'separator' && invertBg ? ' [Inverted]' : '';
    ...
    useInput((input, key) => {
        if (hexInputMode) {
            ...
            if (key.escape) {
                ...
                setHexInput(
                    hexInput.slice(0, cursorPos - 1) + hexInput.slice(cursorPos)
                );
            ...
            if (key.escape) {
                ...
                const currentPresetIndex = presetSeparators.findIndex(
              ...
              = currentPresetIndex === 0
                ...
                const newChar
          = presetSeparators[newIndex]?.char
              ?? presetSeparators[0]?.char
              ?? '\uE0B0';
                ...
                updateSeparators(
                    newSeparators,
                    mode === 'separator' ? newInvertBgs : undefined
                );
            } else if (
                (input === 'a' || input === 'A')
                && (mode === 'separator' || separators.length < 3)
            ) {
          ...
          = defaultChar === '\uE0B2' || defaultChar === '\uE0B6';
                ...
                (input === 'd' || input === 'D')
                && (mode !== 'separator' || separators.length > 1)
            ) {
                // Delete current (min 1 for separator, no min for caps)
                const newSeparators = separators.filter((_, i) => i !== selectedIndex);
                const newInvertBgs
          = mode === 'separator'
              ? invertBgs.filter((_, i) => i !== selectedIndex)
              : [];
                updateSeparators(newSeparators, newInvertBgs);
                setSelectedIndex(
                    Math.min(selectedIndex, Math.max(0, newSeparators.length - 1))
                );
        ...
        <Box flexDirection='column'>
            ...
            {hexInputMode ? (
                <Box marginTop={2} flexDirection='column'>
                    ...
                    <Text dimColor>
                        Enter 4 hex digits (0-9, A-F), then press Enter. ESC to cancel.
                    </Text>
                </Box>
            ) : (
                <>
