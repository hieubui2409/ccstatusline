...
export const App: React.FC = () => {
    ...
    = useState<PowerlineFontStatus>({ installed: false });
    const [installingFonts, setInstallingFonts] = useState(false);
    const [fontInstallMessage, setFontInstallMessage] = useState<string | null>(
        null
    );
    const [existingStatusLine, setExistingStatusLine] = useState<string | null>(
        null
    );
    ...
    // Load existing status line
        ...
        void loadSettings().then((loadedSettings) => {
            ...
            setOriginalSettings(
                JSON.parse(JSON.stringify(loadedSettings)) as Settings
            ); // Deep copy
        });
    ...
    const handleInstallSelection = useCallback(
        (command: string, displayName: string, useBunx: boolean) => {
