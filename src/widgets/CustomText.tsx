...
const CustomTextEditor: React.FC<WidgetEditorProps> = ({
  widget,
  onComplete,
  onCancel,
}) => {
  ...
  const stringToGraphemeIndex = (str: string, stringIndex: number): number => {
    ...
    for (let i = 0; i < graphemes.length; i++) {
      if (currentStringIndex >= stringIndex) return i;
      const grapheme = graphemes[i];
      if (grapheme) {
  ...
  useInput((input, key) => {
    if (key.return) {
      ...
      if (currentGraphemeIndex > 0) {
        const newStringIndex = graphemeToStringIndex(
          text,
      ...
      if (cursorPos > 0) {
        const currentGraphemeIndex = stringToGraphemeIndex(text, cursorPos);
        if (currentGraphemeIndex > 0) {
          const deleteFromIndex = graphemeToStringIndex(
            text,
            currentGraphemeIndex - 1,
          );
          const deleteToIndex = graphemeToStringIndex(
            text,
  ...
  return (
    <Box flexDirection="column">
      <Text>{display}</Text>
      <Text dimColor>
        ←→ move cursor, Ctrl+←→ jump to start/end, Enter save, ESC cancel
      </Text>
    </Box>
  );
};
