...
export class CurrentWorkingDirWidget implements Widget {
  ...
  getEditorDisplay(item: WidgetItem): WidgetEditorDisplay {
    const segments = item.metadata?.segments
      ? parseInt(item.metadata.segments, 10)
      : undefined;
    ...
    return {
      ...
      modifierText:
        modifiers.length > 0 ? `(${modifiers.join(", ")})` : undefined,
    };
  }
  ...
  getCustomKeybinds(): CustomKeybind[] {
    return [
      { key: "s", label: "(s)egments (max dirs)", action: "edit-segments" },
...
const CurrentWorkingDirEditor: React.FC<WidgetEditorProps> = ({
  widget,
  onComplete,
  onCancel,
  action,
  ...
  if (action === "edit-segments") {
    return (
      <Box flexDirection="column">
        <Box>
          <Text>
            Enter number of segments to display (blank for full path):{" "}
          </Text>
