...
const CustomCommandEditor: React.FC<WidgetEditorProps> = ({
    widget,
    onComplete,
    onCancel,
    action
    ...
    const [widthInput, setWidthInput] = useState(
        widget.maxWidth?.toString() ?? ''
    );
    const [timeoutInput, setTimeoutInput] = useState(
        widget.timeout?.toString() ?? '1000'
    );
    ...
    useInput((input, key) => {
        if (mode === 'command') {
            if (key.return) {
                ...
                setCommandCursorPos(
                    Math.min(commandInput.length, commandCursorPos + 1)
                );
            } else if (key.backspace) {
                if (commandCursorPos > 0) {
                    setCommandInput(
                        commandInput.slice(0, commandCursorPos - 1)
                        + commandInput.slice(commandCursorPos)
                    );
