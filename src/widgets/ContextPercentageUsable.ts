  ...
  render(
    ...
    if (context.isPreview) {
      ...
      const usedPercentage = Math.min(
        100,
        (context.tokenMetrics.contextLength / contextConfig.usableTokens) * 100,
      );
      const displayPercentage = isInverse
        ? 100 - usedPercentage
        : usedPercentage;
      return item.rawValue
        ? `${displayPercentage.toFixed(1)}%`
        : `Ctx(u): ${displayPercentage.toFixed(1)}%`;
    }
    return null;
  }

  getCustomKeybinds(): CustomKeybind[] {
    return [{ key: "l", label: "(l)eft/remaining", action: "toggle-inverse" }];
  }
