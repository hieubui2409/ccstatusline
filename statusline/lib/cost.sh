...
# ============================================================================
# Instead of running ccusage commands sequentially (~40s total), run them
# in parallel background processes at script start and collect results later.
# This reduces total wait time to ~10s (single slowest command).

# Prefetch state tracking
export CCUSAGE_PREFETCH_DIR="${COST_CACHE_DIR}/prefetch"
declare -g CCUSAGE_PREFETCH_PIDS=()
declare -g CCUSAGE_PREFETCH_FILES=()
declare -g CCUSAGE_PREFETCH_STARTED=""

# Start parallel prefetch of all ccusage data
start_ccusage_prefetch() {
    [[ "$CCUSAGE_PREFETCH_STARTED" == "true" ]] && return 0

    # Skip if ccusage not available (check without caching for speed)
    if ! command_exists bunx; then
        debug_log "Prefetch skipped: bunx not available" "INFO"
        return 1
    fi

    # Create prefetch directory
    mkdir -p "$CCUSAGE_PREFETCH_DIR" 2>/dev/null
    chmod 700 "$CCUSAGE_PREFETCH_DIR" 2>/dev/null
    ...
    if date -d '7 days ago' "+%Y%m%d" >/dev/null 2>&1; then
        ...
        seven_days_ago=$(date "+%Y%m%d")
        thirty_days_ago=$(date "+%Y%m%d")
    fi

    local prefetch_id="$$_$(date +%s)"
    local timeout_cmd=""
    command_exists timeout && timeout_cmd="timeout ${CONFIG_CCUSAGE_TIMEOUT:-15}"
    command_exists gtimeout && timeout_cmd="gtimeout ${CONFIG_CCUSAGE_TIMEOUT:-15}"

    # Start all commands in parallel
    local blocks_file="$CCUSAGE_PREFETCH_DIR/blocks_${prefetch_id}.json"
    local session_file="$CCUSAGE_PREFETCH_DIR/session_${prefetch_id}.json"
    local daily_file="$CCUSAGE_PREFETCH_DIR/daily_${prefetch_id}.json"
    local monthly_file="$CCUSAGE_PREFETCH_DIR/monthly_${prefetch_id}.json"

    # Launch 4 parallel background processes
    ( $timeout_cmd bunx ccusage blocks --active --json 2>/dev/null > "$blocks_file" ) &
    local pid1=$!

    ( $timeout_cmd bunx ccusage session --since "$seven_days_ago" --json 2>/dev/null > "$session_file" ) &
    local pid2=$!

    ( $timeout_cmd bunx ccusage daily --since "$seven_days_ago" --json 2>/dev/null > "$daily_file" ) &
    local pid3=$!

    ( $timeout_cmd bunx ccusage daily --since "$thirty_days_ago" --json 2>/dev/null > "$monthly_file" ) &
    local pid4=$!

    # Store PIDs and file paths
    CCUSAGE_PREFETCH_PIDS=($pid1 $pid2 $pid3 $pid4)
    CCUSAGE_PREFETCH_FILES=("$blocks_file" "$session_file" "$daily_file" "$monthly_file")
    CCUSAGE_PREFETCH_STARTED="true"

    debug_log "Prefetch started: PIDs=${CCUSAGE_PREFETCH_PIDS[*]}" "INFO"
}

# Wait for prefetch to complete and return results
# Returns: blocks_data:session_data:daily_data:monthly_data (newline-separated)
collect_ccusage_prefetch() {
    if [[ "$CCUSAGE_PREFETCH_STARTED" != "true" ]]; then
        debug_log "Prefetch not started, running sequential fallback" "WARN"
        return 1
    fi

    debug_log "Waiting for prefetch completion..." "INFO"

    # Wait for all background processes with timeout
    local max_wait=20
    local waited=0

    for pid in "${CCUSAGE_PREFETCH_PIDS[@]}"; do
    ...
    local blocks_data="" session_data="" daily_data="" monthly_data=""

    if [[ -s "${CCUSAGE_PREFETCH_FILES[0]}" ]]; then
        blocks_data=$(cat "${CCUSAGE_PREFETCH_FILES[0]}" 2>/dev/null)
    fi
    if [[ -s "${CCUSAGE_PREFETCH_FILES[1]}" ]]; then
        session_data=$(cat "${CCUSAGE_PREFETCH_FILES[1]}" 2>/dev/null)
    fi
    if [[ -s "${CCUSAGE_PREFETCH_FILES[2]}" ]]; then
        daily_data=$(cat "${CCUSAGE_PREFETCH_FILES[2]}" 2>/dev/null)
    fi
    if [[ -s "${CCUSAGE_PREFETCH_FILES[3]}" ]]; then
        monthly_data=$(cat "${CCUSAGE_PREFETCH_FILES[3]}" 2>/dev/null)
    fi

    # Cleanup prefetch files
    rm -f "${CCUSAGE_PREFETCH_FILES[@]}" 2>/dev/null

    # Export results as environment variables for get_claude_usage_info
    export PREFETCH_BLOCKS_DATA="$blocks_data"
    export PREFETCH_SESSION_DATA="$session_data"
    export PREFETCH_DAILY_DATA="$daily_data"
    export PREFETCH_MONTHLY_DATA="$monthly_data"

    debug_log "Prefetch collected in ${waited}x0.5s" "INFO"
    return 0
}

# Cleanup old prefetch files (older than 5 minutes)
cleanup_prefetch_files() {
    [[ -d "$CCUSAGE_PREFETCH_DIR" ]] || return 0
    find "$CCUSAGE_PREFETCH_DIR" -name "*.json" -mmin +5 -delete 2>/dev/null || true
}
...
# Optimized to use parallel prefetch when available
get_claude_usage_info() {
    ...
    # Try to use prefetched data first (parallel optimization)
    if collect_ccusage_prefetch 2>/dev/null; then
        debug_log "Using prefetched cost data (parallel)" "INFO"
        block_data="$PREFETCH_BLOCKS_DATA"
        session_data="$PREFETCH_SESSION_DATA"
        daily_data="$PREFETCH_DAILY_DATA"
        monthly_data="$PREFETCH_MONTHLY_DATA"

        # Update cache files with fresh prefetch data for future requests
        [[ -n "$block_data" ]] && echo "$block_data" > "$BLOCKS_CACHE_FILE" 2>/dev/null
        [[ -n "$session_data" ]] && echo "$session_data" > "$SESSION_CACHE_FILE" 2>/dev/null
        [[ -n "$daily_data" ]] && echo "$daily_data" > "$DAILY_CACHE_FILE" 2>/dev/null
        [[ -n "$monthly_data" ]] && echo "$monthly_data" > "$MONTHLY_CACHE_FILE" 2>/dev/null
    else
        # Fallback to sequential fetching (cached)
        debug_log "Fetching cost data sequentially..." "INFO"
...
# Export prefetch optimization functions
export -f start_ccusage_prefetch collect_ccusage_prefetch cleanup_prefetch_files