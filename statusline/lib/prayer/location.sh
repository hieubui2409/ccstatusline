...
# ============================================================================
# GEOCLUE2 FAILURE CACHING
# ============================================================================
# Cache geoclue2 failures to avoid repeated 15s timeouts

GEOCLUE_FAILURE_CACHE="${STATUSLINE_CACHE_DIR:-${HOME}/.cache/claude-code-statusline}/geoclue_failure.cache"
GEOCLUE_FAILURE_TTL=86400  # 24 hours

# Check if geoclue2 recently failed
is_geoclue_recently_failed() {
    [[ ! -f "$GEOCLUE_FAILURE_CACHE" ]] && return 1

    local cache_mtime failure_age
    cache_mtime=$(get_file_mtime "$GEOCLUE_FAILURE_CACHE" 2>/dev/null || echo 0)
    failure_age=$(($(date +%s) - cache_mtime))

    if [[ $failure_age -lt $GEOCLUE_FAILURE_TTL ]]; then
        debug_log "geoclue2 failure cached (${failure_age}s ago), skipping" "INFO"
        return 0
    fi

    rm -f "$GEOCLUE_FAILURE_CACHE" 2>/dev/null
    return 1
}

# Mark geoclue2 as failed
mark_geoclue_failed() {
    local cache_dir="${STATUSLINE_CACHE_DIR:-${HOME}/.cache/claude-code-statusline}"
    mkdir -p "$cache_dir" 2>/dev/null
    touch "$GEOCLUE_FAILURE_CACHE" 2>/dev/null
    debug_log "Marked geoclue2 as failed for 24h" "INFO"
}
...
get_local_system_coordinates() {
    ...
    if is_geoclue_recently_failed; then
        return 1
    fi
...
get_location_coordinates() {
    ...
    case "$CONFIG_PRAYER_LOCATION_MODE" in
        ...
        "auto"|*)
            ...
            # Tier 1: Try cached location data (FASTEST - moved from Tier 3)
            debug_log "Tier 1: Attempting to load cached location..." "INFO"
            ...
            if cached_location=$(load_cached_location); then
                ...
                debug_log "Tier 1 failed: No valid cached location" "WARN"
            fi

            # Tier 2: Try Local System GPS (Most Accurate)
            debug_log "Tier 2: Attempting local system GPS..." "INFO"
            if coordinates=$(get_local_system_coordinates); then
                export_location_data "$coordinates" "local_gps" "GPS_DEVICE"
                echo "$coordinates"
                debug_log "Local GPS successful: $coordinates" "INFO"
                ...
                debug_log "Tier 2 failed: Local GPS unsuccessful" "WARN"
            fi

            # Tier 3: Try IP geolocation (Network Fallback)
            if check_internet_connection; then
                debug_log "Tier 3: Attempting IP geolocation..." "INFO"
                local location_data
                if location_data=$(get_ip_location); then
                    ...
                    debug_log "Tier 3 failed: IP geolocation unsuccessful" "WARN"
                fi
            else
                debug_log "Tier 3 skipped: No internet connection" "INFO"
            fi
