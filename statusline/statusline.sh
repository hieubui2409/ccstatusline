...
# Required for printf with floating-point numbers from ccusage API
export LC_NUMERIC=C
...
# ============================================================================
# EARLY ARGUMENT PARSING (Fast path for info commands)
# ============================================================================
# Handle simple commands before loading any modules for instant response

case "${1:-}" in
    "--quick-version"|"-qv")
        echo "Claude Code Statusline v2.13.0"
        exit 0
        ;;
    "--quick-help"|"-qh")
        cat <<'EOF'
Claude Code Statusline (Quick Help)

QUICK OPTIONS (instant):
    --quick-version, -qv    Show version instantly
    --quick-help, -qh       Show this quick help

FULL OPTIONS (requires module loading):
    --help, -h              Full help with all options
    --version, -v           Detailed version with module info

For full help: statusline.sh --help
EOF
        exit 0
        ;;
esac
...
# Start ccusage prefetch immediately after cost module loads
# This launches 4 ccusage background processes while other modules load
if is_module_loaded "cost" && declare -f start_ccusage_prefetch >/dev/null 2>&1; then
    start_ccusage_prefetch
    debug_log "ccusage prefetch started (4 parallel background processes)" "INFO"
fi
